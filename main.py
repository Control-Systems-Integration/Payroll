...
try:
    ...
    # Check if errors_df is not empty
    if not errors_df.empty:
        # Reorder the columns in errors_df
        errors_df = errors_df.reindex(columns=column_order)

        # Concatenate merged_df and errors_df
        merged_df = pd.concat([merged_df, errors_df])
        ...
        for row in sheet.iter_rows(min_row=2, min_col=4, max_col=7):  # Modify max_col to 7 for 'Payroll' sheet
            for cell in row:
                if cell.column_letter == 'C' and (cell.value is None or cell.value == ''):
                    ...
                    cell.font = red_font
                elif cell.column_letter == 'G' and cell.value is not None and int(cell.value) < 8:
                    cell.font = red_font
        ...
        for cell in row:
            ...
            if cell.value is not None:
                try:
                    ...
                    if cell_value > 0 and (overtime_start_date is None or overtime_end_date is None):
                        cell.font = red_font
                except ValueError:
                    pass  # Ignore non-integer values
            else:
                cell.font = None  # Reset the font if the value is None
    ...
    #                                                      ignore_index=True)
    for name, group in grouped:
        total_hours = group['Actual Hours Worked'].sum()
