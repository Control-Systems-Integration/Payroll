...
try:
    ...
    ################################################
    # INIT 40 HOURS FEATURE

    # Filepath where the program will save the weekly resume
    filepath = 'C:/test/PayrollWeekly.xlsx'
    ...
    for name, group_name in grouped_by_name:
        ...
        employee_week = pd.DataFrame({
            'Employee Name': [name[0], 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
        ...
        for date, group in grouped_by_name_by_date:
            ...
            if worked_hours_needed == 0:
                employee_week.loc[employee_week['Employee Name'] == week_day,
        ...
        for name_area, group in grouped_by_name_by_area:
            ...
            row = pd.DataFrame(
                {"Employee Name": [new_row[0]], "Sum of Regular Time": [new_row[1]], "Sum of Overtime": [new_row[2]]})
            i += 1
            employee_week = pd.concat([employee_week.iloc[:i], row,
                                       employee_week.iloc[i:]]).reset_index(drop=True)

        wb_weekly_data.create_sheet(name[0])
